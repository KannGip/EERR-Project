...
public class PrototypeGenerator : MonoBehaviour
{
    ...
    public List<Prototype> specialEarthquakePrefabs;
    public List<Prototype> earthquakePrototypes = new List<Prototype>();
    ...
    public void GeneratePrototypes()
    {
        ...
        earthquakePrototypes.Clear();
        for (int i = 0; i < specialEarthquakePrefabs.Count; i++)
        {
            for (int j = 0; j < 4; j++)
            {
                Prototype p = CreateMyAsset(path, "Earthquake_" + specialEarthquakePrefabs[i].name, j.ToString());
                p.prefab = specialEarthquakePrefabs[i].prefab;
                p.meshRotation = j;
                p.attributes = specialEarthquakePrefabs[i].attributes;
                CopyAndRotateSockets(p, specialEarthquakePrefabs[i], j);
                p.directions = CalculateDirectionsFromSockets(p);
                earthquakePrototypes.Add(ScriptableObject.Instantiate(p));
            }
        }
        UpdatePrototypes();
